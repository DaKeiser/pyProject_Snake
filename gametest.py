...
#lad high score
    ...
    with open(path.join(dir, HS_FILE), 'w') as f:   #Command reqd. to open the file
        try :
            highscore = int(f.read()) #read score as a string and convert it into a number
        except:
            #Incase the file is empty then fill it with 0
            highscore = 0
...
def instructions():
  ...
  msg = "Use UP arrow key to move up. \n Use DOWN arrow key to move down. \n Use RIGHT and LEFT arrow keys to move right and left respectively. \n If the snake's head hits it's own body or the walls the game is over."
